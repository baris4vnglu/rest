...
// MASA LİSTESİ FIREBASE'DEN ÇEKİLİYOR
...
// REZERVASYON EKLEME
reservationForm1.addEventListener("submit", async (e) => {
    e.preventDefault();

    // Tarih kontrolü (çalışan sürüm)
    const selectedDate = document.getElementById("res-date").value;
    const selectedParts = selectedDate.split("-");
    const selectedObj = new Date(
    ...
    if (selectedObj.getTime() < todayObj.getTime()) {
        alert("Geçmiş bir tarihe rezervasyon yapılamaz!");
        return;
    }
    // SAAT ALANI
    ...
    try {
        await addDoc(collection(db, "reservations"), {
            date: selectedDate,
            time: time,                       // ↩ SAAT BURADA KAYDEDİLİYOR
            table: document.getElementById("res-table").value,
            people: document.getElementById("res-people").value,
            name: document.getElementById("res-name").value,
            createdAt: Timestamp.now()
        });

        reservationForm1.reset();
        loadReservations();
