...
async function showTableDetails(table) {
    ...
    const paymentSection = document.getElementById('table-payment-section');
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const status = statusStyles[order.status] || { bg: '#f3f4f6', text: order.status };
...
async function loadUnpaidOrders() {
    ...
    try {
        ...
        querySnapshot.forEach((doc) => {
            unpaidOrders.push({ id: doc.id, ...doc.data() });
        });
...
function selectOrderForPayment(orderId, amount, isSplit) {
    ...
    document.getElementById('unpaid-orders-list').style.display = 'none';
...
async function processPayment(paymentMethod, amount, orderId, isSplit) {
    ...
    if (isSplit) {
        ...
        paymentData.splitCount = parseInt(document.getElementById('split-count').value) || 2;
    }
    ...
    try {
        ...
        document.getElementById('unpaid-orders-list').style.display = 'block';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load unpaid orders when payment modal is opened
    const paymentModal = document.getElementById('payment-modal');
    if (paymentModal) {
        paymentModal.addEventListener('shown.bs.modal', loadUnpaidOrders);
    }
});
